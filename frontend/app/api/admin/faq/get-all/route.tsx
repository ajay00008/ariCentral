...
export async function GET (): Promise<NextResponse> {
  try {
    ...
    const response: Response = await fetchAPI('/api/faqs?populate=deep&pagination[pageSize]=100&sort[0]=createdAt:asc', { token: sessionToken }, false, true)

    if (response.ok) {
      const faqCollection = await response.json()
      return NextResponse.json(faqCollection.data, { status: 200 })
    } else {
