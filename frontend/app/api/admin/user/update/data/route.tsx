...
export async function POST (request: Request): Promise<NextResponse<ActionCreateUser | APIError>> {
  try {
    ...
    } else if (!idCheck.safeParse(id).success) {
      return NextResponse.json({ message: 'Id should be string.' }, { status: 400 })
    ...
    const response: Response = await fetchAPI(`/api/users/${id.toString()}?populate=deep`, {
      method: 'PUT',
      token: sessionToken,
