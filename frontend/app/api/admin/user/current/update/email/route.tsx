...
export async function POST (request: Request): Promise<NextResponse<ActionUpdateCurrentUserEmail | APIError>> {
  try {
    const { data } = await request.json()
    const { id, email }: { id: number, email: string } = data

    const idZodCheck = z.number()
    const emailZodCheck = z.string()
    ...
    } else if (!emailZodCheck.safeParse(email).success) {
      return NextResponse.json({ message: 'Email body parameter should be string.' }, { status: 400 })
    }
